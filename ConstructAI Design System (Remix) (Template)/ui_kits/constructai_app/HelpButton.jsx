/* global React */
// HelpButton.jsx — floating Manual_Assistant_AI

const FAQS = [
  'Como registrar um RDO?',
  'Como interpretar o CPI?',
  'Como abrir uma NCI?',
  'O que é o ESG Score?',
];

function HelpButton() {
  const [open, setOpen] = React.useState(false);
  const [messages, setMessages] = React.useState([
    { role:'assistant', content:'👋 Olá! Sou o assistente da plataforma. Como posso ajudar você hoje?' },
  ]);
  const [input, setInput] = React.useState('');

  function send(text) {
    const q = (text || input).trim();
    if (!q) return;
    setInput('');
    setMessages(m => [...m, { role:'user', content:q },
      { role:'assistant', content:'(Demo) Em produção, essa pergunta seria roteada para o agente Manual_Assistant_AI. Posso ajudar com mais alguma dúvida sobre a plataforma?' }]);
  }

  return (
    <React.Fragment>
      <button onClick={() => setOpen(o=>!o)} title="Ajuda — Manual da Plataforma"
        style={{
          position:'fixed', bottom:24, right:24, zIndex:9999,
          width:50, height:50, borderRadius:'50%',
          background: open?'#0C447C':'#185FA5', color:'#fff',
          border:'none', cursor:'pointer', fontSize:22, fontWeight:700,
          boxShadow:'0 4px 16px rgba(24,95,165,0.4)',
          display:'flex', alignItems:'center', justifyContent:'center',
        }}>{open?'✕':'?'}</button>

      {open && (
        <div style={{
          position:'fixed', bottom:84, right:24, zIndex:9998,
          width:360, height:480, background:'#fff', borderRadius:16,
          boxShadow:'0 8px 40px rgba(0,0,0,0.18)',
          display:'flex', flexDirection:'column',
          border:'1px solid #e5e8f0', overflow:'hidden',
        }}>
          <div style={{ padding:'14px 16px', background:'#185FA5',
            display:'flex', alignItems:'center', gap:10, flexShrink:0 }}>
            <div style={{ width:32, height:32, borderRadius:'50%',
              background:'rgba(255,255,255,0.2)',
              display:'flex', alignItems:'center', justifyContent:'center', fontSize:16 }}>📖</div>
            <div>
              <div style={{ fontSize:13, fontWeight:600, color:'#fff' }}>Manual da Plataforma</div>
              <div style={{ fontSize:10, color:'rgba(255,255,255,0.7)' }}>ConstructAI v5.3 · Assistente</div>
            </div>
          </div>

          <div style={{ flex:1, overflowY:'auto', padding:'12px 14px',
            display:'flex', flexDirection:'column', gap:10, background:'#f8f9fc' }}>
            {messages.map((m, i) => (
              <div key={i} style={{ display:'flex',
                justifyContent: m.role==='user'?'flex-end':'flex-start' }}>
                <div style={{
                  maxWidth:'85%', padding:'9px 12px',
                  borderRadius: m.role==='user' ? '12px 4px 12px 12px' : '4px 12px 12px 12px',
                  background: m.role==='user' ? '#185FA5' : '#fff',
                  color: m.role==='user' ? '#fff' : '#1a1f36',
                  fontSize:12, lineHeight:1.6,
                  border: m.role==='assistant' ? '1px solid #e5e8f0' : 'none',
                }}>{m.content}</div>
              </div>
            ))}
          </div>

          {messages.length <= 1 && (
            <div style={{ padding:'8px 14px', background:'#fff',
              borderTop:'1px solid #e5e8f0', flexShrink:0 }}>
              <div style={{ fontSize:10, color:'#8b93a7', marginBottom:6,
                fontWeight:500, textTransform:'uppercase', letterSpacing:'.06em' }}>
                Perguntas frequentes
              </div>
              <div style={{ display:'flex', flexWrap:'wrap', gap:5 }}>
                {FAQS.map((q,i) => (
                  <button key={i} onClick={() => send(q)} style={{
                    fontSize:10, padding:'4px 9px', background:'#EFF4FF',
                    color:'#185FA5', border:'1px solid #B5D4F4',
                    borderRadius:20, cursor:'pointer', fontFamily:'inherit',
                  }}>{q}</button>
                ))}
              </div>
            </div>
          )}

          <div style={{ padding:'10px 12px', background:'#fff',
            borderTop:'1px solid #e5e8f0',
            display:'flex', gap:8, flexShrink:0 }}>
            <input value={input} onChange={e => setInput(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && send()}
              placeholder="Tire sua dúvida sobre a plataforma..."
              style={{ flex:1, padding:'8px 12px',
                border:'1px solid #e5e8f0', borderRadius:8,
                fontSize:12, background:'#f8f9fc', color:'#1a1f36', outline:'none' }} />
            <button onClick={() => send()} disabled={!input.trim()}
              style={{ padding:'8px 14px', background:'#185FA5', color:'#fff',
                border:'none', borderRadius:8, cursor:'pointer',
                fontSize:12, fontWeight:600, opacity: input.trim()?1:0.5 }}>→</button>
          </div>
        </div>
      )}
    </React.Fragment>
  );
}

window.HelpButton = HelpButton;
